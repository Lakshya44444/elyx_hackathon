...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int, is_travel_week: bool):
    travel_note = "Rohan is traveling this week (timezone shifts, hotel gyms, flight fatigue, diet challenges)." if is_travel_week else "Rohan is in Singapore (SGT scheduling)."
    return f"""
Week 6, Day {day_idx} ({date_str}).
Generate exactly {n_turns} raw chat lines. Each line format: 'Speaker: message'.
Include:
- Blood panel scheduling this week.
- Exercise adjustments (hotel-friendly).
- HRV dips and sleep guidance.
- Nutrition question about fish oil.
- Curiosity questions from Rohan (≤5 in week).
- Adherence ~50%, so some adjustments.
- {travel_note}
Constraints:
...
def generate_week6(start_date="2025-02-05", daily_min=12, daily_max=18, seed=123):
    random.seed(seed)
    week_json = []
...
if __name__ == "__main__":
    data = generate_week6(start_date="2025-02-05")
    with open("elyx_week6_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week6_communications.json")
