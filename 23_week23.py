...
- One full diagnostic test panel every 3 months (due in Week 24).
...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int, is_travel_week: bool):
    travel_note = "Rohan is traveling this week; include timezone and flight logistics." if is_travel_week else "Rohan is in Singapore; keep scheduling in SGT."
    return f"""
Week 23, Day {day_idx} ({date_str}).
Goal: Generate exactly {n_turns} chat lines, each 'Speaker: message'.
Include:
- Reminders about upcoming Week 24 diagnostic panel (lab prep, fasting, logistics).
- Ongoing plan discussions: exercise updates (biweekly), nutrition tweaks, stress/sleep monitoring.
...
def generate_week23(start_date="2025-05-28", daily_min=12, daily_max=18, seed=23):
    random.seed(seed)
    week_json = []
    system_prompt = build_system_prompt()

    # Week 23: Assume Rohan is at home (non-travel week)
    travel_week = False
...
if __name__ == "__main__":
    data = generate_week23(start_date="2025-05-28")
    with open("elyx_week23_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week23_communications.json")
