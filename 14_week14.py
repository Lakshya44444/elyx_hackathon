...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int):
    return f"""
Week 14, Day {day_idx} ({date_str}).
Generate exactly {n_turns} WhatsApp-style chat lines.
Include:
- Rohan has completed or is mid-way through his test panel.
- Elyx team reviewing test results.
- Discuss insights, next steps, lifestyle interventions, or specialist consults.
- Sarah can speak on behalf of Rohan.
- Concierge nudges to complete any pending forms or tests.
- Short curiosity questions by Rohan.
Constraints:
...
def generate_week14(start_date="2025-04-09", daily_min=12, daily_max=18, seed=1414):
    random.seed(seed)
    week_json = []
...
if __name__ == "__main__":
    data = generate_week14(start_date="2025-04-09")
    with open("elyx_week14_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week14_communications.json")
