...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int):
    return f"""
Week 30, Day {day_idx} ({date_str}).
Generate exactly {n_turns} WhatsApp-style chat lines.
Include:
- Ruby’s weekly check-in (remove blockers, nudge adherence).
- Rohan skipped some hotel workouts last week → Advik adjusts plan.
- Rohan asks curiosity questions: sauna/cold plunge for recovery, magnesium/ashwagandha for sleep, HIIT vs cardio, coffee timing.
- Carla shares dining-out hacks since Rohan slipped at social dinners.
- Rachel reminds: test panel is due after Week 36.
- Rohan is back in Singapore this week (no travel).
- Keep chats natural, WhatsApp-style, short.
"""


def generate_week30(start_date="2025-08-14", daily_min=12, daily_max=16, seed=3030):
    random.seed(seed)
    week_json = []
...
if __name__ == "__main__":
    data = generate_week30(start_date="2025-08-14")
    with open("elyx_week30_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week30_communications.json")
