...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int, is_travel_week: bool):
    travel_note = "Rohan is traveling this week (timezone shifts, flight logistics, hotel gyms)." if is_travel_week else "Rohan is in Singapore (SGT scheduling)."
    return f"""
Week 5, Day {day_idx} ({date_str}).
Generate exactly {n_turns} raw chat lines. Each line format: 'Speaker: message'.
Include:
- Test results (some major issues <act>, some followup <soft follow up>, some all okay <note>).
- Exercise plan updated this week.
- Rohan asks up to 5 curiosity questions during the week.
- Adherence ~50%, so some plan adjustments.
- {travel_note}
Constraints:
...
def generate_week5(start_date="2025-01-29", daily_min=12, daily_max=18, seed=99):
    random.seed(seed)
    week_json = []
    system_prompt = build_system_prompt()

    # Week 5 → NOT travel week (Week 4 was travel)
    travel_week = False
...
if __name__ == "__main__":
    data = generate_week5(start_date="2025-01-29")
    with open("elyx_week5_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week5_communications.json")
