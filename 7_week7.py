...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int, is_travel_week: bool):
    travel_note = "Rohan is back in Singapore this week (normal SGT schedule)." if not is_travel_week else "Rohan is traveling this week (timezone shifts, hotel gyms, diet adjustments)."
    return f"""
Week 7, Day {day_idx} ({date_str}).
Generate exactly {n_turns} raw chat lines. Each line format: 'Speaker: message'.
Include:
- Blood panel results discussed this week (LDL borderline, ApoB high).
- Nutrition adjustments and supplement tweaks recommended.
- Exercise back to normal (post-travel).
- Rohan asks up to 5 curiosity questions across the week.
- Adherence ~50–60%, with a few reminders.
- {travel_note}
Constraints:
...
def generate_week7(start_date="2025-02-12", daily_min=12, daily_max=18, seed=321):
    random.seed(seed)
    week_json = []
    system_prompt = build_system_prompt()

    # Week 7 → NOT a travel week
    travel_week = False
...
if __name__ == "__main__":
    data = generate_week7(start_date="2025-02-12")
    with open("elyx_week7_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week7_communications.json")
