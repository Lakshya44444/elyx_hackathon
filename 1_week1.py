...
ALLOWED_EVENTS = ["update", "question", "test", "plan_change", "travel", "logistics",
                  "followup", "education", "escalation", "resolution", "attachment"]
...
def classify_event(message: str) -> str:
    ...
    rules = [
        ...
        ("test", ["test", "panel", "results", "lab", "report", "ogtt", "ldl",
                  "apo", "lp(a)", "cimt", "mri", "fit"]),
...
def build_system_prompt():
    return f"""
You are the Elyx Concierge Team. Generate WhatsApp-style messages between
Rohan (the member), Sarah (his PA), and Elyx experts.
Rules:
...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int):
    return f"""
Week 1, Day {day_idx} ({date_str}).
This is the onboarding week.
Generate exactly {n_turns} raw chat lines. Each line format: 'Speaker: message'.
Include:
- Rohan onboarding with Elyx team: shares history, priorities, diet.
- Sarah introduces herself to Ruby and helps with setup.
- Ruby coordinates logistics, schedules test panel, reminders.
- Dr. Warren discusses baseline medical strategy.
- Advik mentions wearables / data tracking.
- Carla starts food/nutrition conversation.
- Rachel introduces exercises (first 2-week plan).
- Neel welcomes and gives big-picture reassurance.
- Rohan has ~2-3 curious questions this week.
- Singapore is his residence.
- Rohan has one chronic condition (e.g., high BP).
Constraints:
...
def generate_week1(start_date="2025-01-15", daily_min=10, daily_max=16, seed=101):
    random.seed(seed)
    week_json = []
...
if __name__ == "__main__":
    data = generate_week1(start_date="2025-01-15")
    with open("elyx_week1_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week1_communications.json")
