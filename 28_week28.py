...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int):
    return f"""
Week 28, Day {day_idx} ({date_str}).
Generate exactly {n_turns} WhatsApp-style chat lines.
Include:
- New workout program update from Advik (every 2 weeks).
- Ruby nudges Rohan about ~50% adherence, suggests small swaps/tweaks.
- Curiosity questions from Rohan (about fasting vs eating pre-workout, supplements for brain health, sleep debt, HRV tracking, sauna).
- Rachel reminds that full diagnostic test panel is due after Week 36.
- No travel this week (stable routine in Singapore).
- Sarah helps with scheduling/logistics.
- Keep messages natural, short, WhatsApp-style.
"""


def generate_week28(start_date="2025-07-31", daily_min=12, daily_max=16, seed=2828):
    random.seed(seed)
    week_json = []
...
if __name__ == "__main__":
    data = generate_week28(start_date="2025-07-31")
    with open("elyx_week28_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week28_communications.json")
