...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int, is_travel_week: bool):
    ...
    return f"""
Week 8, Day {day_idx} ({date_str}).
Generate exactly {n_turns} raw chat lines. Each line format: 'Speaker: message'.
Include:
- Diagnostics focus this week: CIMT + MRI scheduled/discussed, some labs too.
- Some reassuring results, some needing follow-up.
- A couple of logistics/scheduling chats.
- Nutrition/exercise adherence ~50–60%.
...
def generate_week8(start_date="2025-02-19", daily_min=12, daily_max=18, seed=456):
    random.seed(seed)
    week_json = []
    system_prompt = build_system_prompt()

    # Week 8 → NOT a travel week
    travel_week = False
...
if __name__ == "__main__":
    data = generate_week8(start_date="2025-02-19")
    with open("elyx_week8_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week8_communications.json")
