...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int):
    return f"""
Week 32, Day {day_idx} ({date_str}).
This is the final week of the 8-month program.
Generate exactly {n_turns} raw chat lines. Each line format: 'Speaker: message'.
Include:
- Reflections on progress so far, reminders, encouragement.
- Sarah sometimes speaking on behalf of Rohan with Elyx team.
- Rohan curious with 2–3 questions during the week.
- Some exercise/lifestyle updates and nudges.
- Plan adherence ~50%, some adjustments still needed.
Constraints:
...
def generate_week32(start_date="2025-09-10", daily_min=10, daily_max=16, seed=321):
    random.seed(seed)
    week_json = []
...
if __name__ == "__main__":
    data = generate_week32(start_date="2025-09-10")
    with open("elyx_week32_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week32_communications.json")
