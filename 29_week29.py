...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int):
    return f"""
Week 29, Day {day_idx} ({date_str}).
Generate exactly {n_turns} WhatsApp-style chat lines.
Include:
- Ruby’s weekly check-in (remove blockers, nudge adherence).
- Rohan follows ~50% of plan, Ruby tweaks where needed.
- Rohan asks curiosity questions (diet timing, morning vs evening workouts, nootropics, sleep supplements, intermittent fasting & exercise).
- Rachel reminds test panel is after Week 36.
- This week Rohan is traveling for business to Hong Kong → logistics by Sarah.
- Keep chats natural, WhatsApp-style, short.
"""


def generate_week29(start_date="2025-08-07", daily_min=12, daily_max=16, seed=2929):
    random.seed(seed)
    week_json = []
...
if __name__ == "__main__":
    data = generate_week29(start_date="2025-08-07")
    with open("elyx_week29_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week29_communications.json")
