...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int):
    return f"""
Week 15, Day {day_idx} ({date_str}).
Generate exactly {n_turns} WhatsApp-style chat lines.
Include:
- Follow-up on Week 14 test results and insights.
- Discuss next steps, interventions, specialist consults.
- Rohan or Sarah can ask clarification questions about results.
- Concierge nudges for pending appointments, lifestyle tracking, medication adherence.
- Experts providing guidance, tips, or education based on test results.
- Short motivational messages to keep Rohan on track.
Constraints:
...
def generate_week15(start_date="2025-04-16", daily_min=12, daily_max=18, seed=1515):
    random.seed(seed)
    week_json = []
...
if __name__ == "__main__":
    data = generate_week15(start_date="2025-04-16")
    with open("elyx_week15_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week15_communications.json")
