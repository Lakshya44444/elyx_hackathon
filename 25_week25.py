...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int):
    return f"""
Week 25, Day {day_idx} ({date_str}).
Generate exactly {n_turns} WhatsApp-style chat lines.
Include:
- Physicians review test results from Week 24 and share feedback.
- Ruby helps coordinate logistics and sends summaries.
- Adjustments to nutrition, supplements, and exercise plan based on results.
- 4–5 curiosity questions from Rohan on health topics (ApoE4, microbiome, inflammation, hormones).
- Bi-weekly exercise update continues.
- Member follows ~50% of the plan, team nudges and adapts.
Constraints:
...
def generate_week25(start_date="2025-07-10", daily_min=12, daily_max=16, seed=2025):
    random.seed(seed)
    week_json = []
...
if __name__ == "__main__":
    data = generate_week25(start_date="2025-07-10")
    with open("elyx_week25_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week25_communications.json")
