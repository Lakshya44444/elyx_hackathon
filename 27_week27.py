...
- Travel: At least 1 week out of every 4 on business trips (UK/US/KR/Jakarta time zones)
...
- One full diagnostic test panel every 3 months (Week 24 just done, next due Week 36).
...
- Member travels 1 week out of every 4 (Week 27 is a travel week).
...
def classify_event(message: str) -> str:
    ...
    rules = [
        ...
        ("test", ["test", "panel", "results", "lab",
         "report", "bloodwork", "diagnostic"]),
...
def build_system_prompt():
    return f"""
You are the Elyx Concierge Team. Generate WhatsApp-style messages between Rohan Patel, his PA (Sarah), and Elyx experts.
Rules:
- Keep each message 1–2 short lines, WhatsApp tone.
- Use only these speakers: {", ".join(ALLOWED_SENDERS)}.
- No markdown, no numbering, no explanations—only chat content.
Context:
{TEAM_ROLES}
...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int, is_travel_week: bool):
    travel_note = "Rohan is on a business trip this week (flights, timezone changes, hotel workouts, meal constraints)." if is_travel_week else "Rohan is in Singapore; keep scheduling in SGT."
    return f"""
Week 27, Day {day_idx} ({date_str}).
Goal: Generate exactly {n_turns} chat lines, each 'Speaker: message'.
Include:
- Travel logistics, timezone coordination, flight/jet lag adjustments.
- Exercise updates (biweekly plan refresh).
- Nutrition tweaks for eating during travel.
- Up to 5 Rohan curiosity questions sprinkled across week.
- Adherence ~50% → some plan adjustments required.
- {travel_note}
Constraints:
- Very short, WhatsApp-style messages.
- Use only allowed speakers.
- No timestamps in raw output, only 'Speaker: text'.
"""

# ------------ Core Generator ------------


def generate_week27(start_date="2025-06-25", daily_min=12, daily_max=18, seed=27):
    random.seed(seed)
    week_json = []
    system_prompt = build_system_prompt()

    travel_week = True  # Week 27 = travel week
    ...
    for d in range(7):
        ...
        for ln in lines:
            ...
            if not m:
                continue
            sender, message = m.group(1), m.group(2).strip()
            if not message:
                continue
...
if __name__ == "__main__":
    data = generate_week27(start_date="2025-06-25")
    with open("elyx_week27_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week27_communications.json")
