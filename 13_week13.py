...
def classify_event(message: str) -> str:
    ...
    rules = [
        ...
        ("test", ["test", "panel", "results", "lab", "report", "ogtt", "ldl", "apo",
         "lp(a)", "cimt", "mri", "fit", "dexa", "cortisol", "thyroid", "urinalysis"]),
...
def build_user_prompt(day_idx: int, date_str: str, n_turns: int):
    return f"""
Week 13, Day {day_idx} ({date_str}).
Generate exactly {n_turns} WhatsApp-style chat lines.
Include:
- Discuss Rohan's upcoming comprehensive Test Panel.
- Explain rationale for key tests (General Health, Cancer Screening, Cardiovascular, Genetics, Hormones, Body Composition, Brain, Skin, Nutritional).
- Ask Rohan/Sarah for consent, schedule, preferences.
- Some nudges by concierge to complete tasks.
- Short curiosity questions by Rohan (up to 5 this week).
- Sarah can speak on behalf of Rohan.
Constraints:
...
def generate_week13(start_date="2025-04-02", daily_min=12, daily_max=18, seed=1313):
    random.seed(seed)
    week_json = []
...
if __name__ == "__main__":
    data = generate_week13(start_date="2025-04-02")
    with open("elyx_week13_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week13_communications.json")
