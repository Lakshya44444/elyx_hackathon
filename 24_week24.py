...
- Sarah may speak on behalf of Rohan to team members for logistics or scheduling.
- Keep exchanges realistic, WhatsApp-like.
"""


def build_user_prompt(day_idx: int, date_str: str, n_turns: int):
    return f"""
Week 24, Day {day_idx} ({date_str}).
Generate exactly {n_turns} WhatsApp-style chat lines.
Include:
- Diagnostics: full test panel scheduled (end of 3-month cycle).
- Conversations about logistics, booking labs, preparing Rohan.
- 4–5 curiosity questions by Rohan during the week about health topics (genetics, microbiome, biomarkers).
- Chronic condition management: sugar/BP monitoring.
- Exercise updates bi-weekly.
- Elyx team explains rationale, follows up, motivates.
- Member follows plan ~50% of time, adjustments suggested.
Constraints:
...
def generate_week24(start_date="2025-07-03", daily_min=13, daily_max=18, seed=2024):
    random.seed(seed)
    week_json = []
...
if __name__ == "__main__":
    data = generate_week24(start_date="2025-07-03")
    with open("elyx_week24_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week24_communications.json")
