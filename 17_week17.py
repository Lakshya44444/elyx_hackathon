...
- Short messages only, no numbering or markdown.
"""


def build_user_prompt(day_idx: int, date_str: str, n_turns: int):
    return f"""
Week 17, Day {day_idx} ({date_str}).
Generate exactly {n_turns} WhatsApp-style chat lines.
Include:
- Follow-up on previous interventions and exercise updates.
- 1 full diagnostic test panel done this week (progress tracking, highlighted issues).
- Rohan or Sarah may ask 1–2 curiosity or research questions per day on health topics.
- Concierges push for adherence; member sticks to plan ~50% of time, adjust as needed.
- Motivational messages and educational tips from experts.
- Include travel if relevant (1 week business trip scenario possible).
Constraints:
- Short WhatsApp messages only.
- Allowed speakers only.
- Reflect member in Singapore managing 1 chronic condition (e.g., high sugar or BP).
"""

# ------------ Generator ------------


def generate_week17(start_date="2025-05-01", daily_min=12, daily_max=18, seed=1717):
    random.seed(seed)
    week_json = []
    ...
    for d in range(7):
        ...
        # Assign timestamps and classify events
...
if __name__ == "__main__":
    data = generate_week17(start_date="2025-05-01")
    with open("elyx_week17_communications.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("✅ Generated: elyx_week17_communications.json")
